...
export function avviaMotoreDDS(db, auth) {
    ...
    window.apriModaleAggiungiDDS = () => {
        ...
        // Logica per mostrare o nascondere l'input notifiche (App vs Web)
        ...
        // Inizializza flatpickr in sicurezza se la libreria è pronta
        if (!fpInstance && window.flatpickr) {
            fpInstance = window.flatpickr("#dds-date-picker", { 
                mode: "multiple", 
                dateFormat: "Y-m-d", 
                locale: "it", 
                altInput: true, 
        ...
        window.apriModal('modal-aggiungi-dds');
    };
    ...
    window.salvaDDS = () => {
        ...
        if (!fpInstance || !fpInstance.selectedDates) {
        ...
        reader.onload = function(e) {
            ...
            reqAdd.onsuccess = async function() {
                ...
                if (auth.currentUser && (bannerAttiva || (orarioPush && (isNative || isWeb)))) {
                    try {
                ...
                window.chiudiModal('modal-aggiungi-dds');
                window.caricaListaDDS();
    ...
    window.caricaListaDDS = () => {
        ...
        reqGetAll.onsuccess = function() {
            ...
            if (ddsArray.length === 0) { 
                listContainer.innerHTML = '<div class="dds-empty-state"><i class="fa-solid fa-folder-open" style="font-size:32px; color:var(--border-color); display:block; margin-bottom:10px;"></i> Nessuna DDS archiviata.</div>'; 
                return; 
            }
            ...
            ddsArray.forEach(dds => {
                ...
                card.innerHTML = `
                    ...
                    <div class="dds-btn-group">
                        <button class="dds-btn-open" onclick="window.apriPDFDDS(${dds.id})"><i class="fa-solid fa-eye"></i> Apri PDF</button>
                        <button class="dds-btn-delete" onclick="window.eliminaDDS(${dds.id})"><i class="fa-solid fa-trash-can"></i> Elimina</button>
                    </div>`;
    ...
    window.importaDDS = (event) => {
        ...
        event.target.value = ''; // Resetta l'input file per permettere un'altra importazione
    };
}
