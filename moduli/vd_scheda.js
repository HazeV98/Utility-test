    ...
    mediaArray.forEach((path, index) => {
        ...
        if (isPdf) {
            ...
            const actionBtn = isEditMode
                ...
                : `<button class="btn-download-media icon-btn" style="position: absolute; top: 5px; right: 5px; background: var(--surface); color: var(--text-main); width: 30px; height: 30px; border-radius: 50%; justify-content: center; align-items: center; border: 1px solid var(--border-color); z-index: 10; box-shadow: none;" onclick="window.Scheda.scaricaFile('${urlRelativo}', '${filename}')" title="Scarica"><i class="fa-solid fa-download" style="font-size: 12px;"></i></button>`;
...
function apriViewer(index) {
    ...
    if (ext === 'pdf') {
        window.open(urlRelativo, '_blank');
        return;
    }
